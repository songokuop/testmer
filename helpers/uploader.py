        ...
        async with userBot:
            if upload_mode is False:
                ...
                sent_: Message = await userBot.send_document(
                    ...
                    thumb=video_thumbnail,
                    caption=f"`{merged_video_path.rsplit('/',1)[-1]}`\n\nMᴇʀɢᴇᴅ ғᴏʀ: <a href='tg://user?id={cb.from_user.id}'>{cb.from_user.first_name}</a>",
                    progress=prog.progress_for_pyrogram,
                    progress_args=(
                        f"Uᴘʟᴏᴀᴅɪɴɢ: `{merged_video_path.rsplit('/',1)[-1]}`",
                        c_time,
    ...
    try:
        ...
        if sent_ is not None:
            if Config.LOGCHANNEL is not None:
                ...
                await sent_.copy(
                    chat_id=int(LOGCHANNEL),
                    caption=f"`{media.file_name}`\n\nExᴛʀᴀᴄᴛᴇᴅ ʙʏ: <a href='tg://user?id={cb.from_user.id}'>{cb.from_user.first_name}</a>",
                )
    except:
        1    
    1
